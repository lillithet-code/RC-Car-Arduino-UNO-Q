...
All drivers can use **W/S** or **Up/Down** to drive and **A/D** or **Left/Right** to steer, including combined keys and fullscreen mode. Release keys to stop; changing tabs or losing focus also stops control. Keyboard driving is disabled while using form controls. The joystick remains available and takes over when touched.
...
- `PICAMERA_IMX219_FRAME_RATE`, `PICAMERA_IMX219_VIDEO_WIDTH`, `PICAMERA_IMX219_VIDEO_HEIGHT`: IMX219-specific capture settings used when `PICAMERA_SENSOR=imx219`
- `LIBCAMERA_BIN`: optional override camera binary path/name; auto-detects `rpicam-vid` then `libcamera-vid`
- The Pi CSI camera stream is rotated 180 degrees at capture to match the camera mounting orientation.
- `GPIO_DRY_RUN`: if `1`, logs commands without toggling GPIO
- `DRIVE_IN1_PIN`, `DRIVE_IN2_PIN`: DRV8871 drivetrain PWM pins (defaults `12`, `13`)
- `SERVO_PIN`: steering servo signal pin (software PWM generated by board runtime; default `6`)
...
- `LIGHTS_PIN`: lights control pin (default `5`)
...
- IMX219 uses its own 1280x720@30 defaults unless you override the IMX219-specific variables above.
